...
function response2(id_actor){
    ...
    if(tab.includes(res[res.length-1].value)){
      ...
      tab.push(res[res.length-1].value);
      call_films_id(id_actor,res[res.length-1].value);
    }
    
}

function call_films_id(id_actor,rep){
  console.log(id_actor);
...
function compare_movie(data,movie){
  let i=0;
  let exist=0;
  let casts=data.cast;
  let baseUrl = 'https://www.themoviedb.org/t/p/w600_and_h900_bestv2'
